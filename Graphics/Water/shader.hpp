...
GLuint LoadShaders(const char* vertex_file_path, const char* geo_file_path, const char* fragment_file_path) {
    ...
    std::ifstream TCSStream("TessExample.tcs", std::ios::in);
    ...
    std::ifstream TESStream("TessExample.tes", std::ios::in);
...
GLuint LoadShadersAll(string file) {

    string vertex_file_path = file + ".vertexshader";
    string tcs_file_path = file + ".tcs";
    string tes_file_path = file + ".tes";
    string geo_file_path = file + ".geoshader";
    string fragment_file_path = file + ".fragmentshader";
    ...
    // Read the Fragment Shader code from the file
    std::string TCSshader;
    std::ifstream TCSStream(tcs_file_path, std::ios::in);
    if (TCSStream.is_open()) {
        std::stringstream sstr;
        sstr << TCSStream.rdbuf();
        TCSshader = sstr.str();
    ...
    // Read the Fragment Shader code from the file
    std::string TESshader;
    std::ifstream TESStream(tes_file_path, std::ios::in);
    if (TESStream.is_open()) {
        std::stringstream sstr;
        sstr << TESStream.rdbuf();
        TESshader = sstr.str();
